...
function KssCompiler (templateTree, stylesDir, appCssUrl, options) {
  ...
  // Temporary hardcoded path to the main app, generate templates in templates/catalogue/
...
KssCompiler.prototype.updateCache = function (srcDir, destDir) {
  ...
  return parseStyleguide(this.stylesDir)
      .then(function(styleguide) {
        ...
        // Add the /catalogue folder in /app
        ...
        try {
          ...
          for (var i = 0; i < sectionRoots.length; i++) {
            ...
            // Before you generate the page run process to run bl-route in the command line
          ...
          generateIndex(outputDestDir, self.stylesDir + '/styleguide.md', styleguide, childSections, pages, sectionRoots, self.appCssUrl);
        } catch (err) {
...
function getFilteredMarkup(markup) {
  ...
  // Need to unencode this
  return escape(htmlOutput);
}
...
function generateIndex(outputDir, styleguideMdPath, styleguide, sections, pages, sectionRoots, appCssUrl) {
  ...
  try {
    fs.writeFileSync(outputDir + '/index.hbs',
        template({
          styleguide: styleguide,
          sectionRoots: sectionRoots,
          sections: jsonSections(sections),
